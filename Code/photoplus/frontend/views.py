...
def buy(request, idP, resolution):    
    ...
    if request.POST:
	    ...
	    post_url =  photo_id.post_url 
 	    #message = u'Hello ' + cd['FirstName'] + u', \n We are sending you a quick email to confirm all the details of your order.\n\n And we will deliver to: '+ cd['FirstName'] + u', '+ cd['LastName'] + u', '+  cd['Country']
            
            
            c = Context({"f_name": f_name, "l_name": l_name, "country": country, "adress": adress, "index": index, "price": price, "size": size, "post_title":post_title, "image_url":image_url, "post_url": post_url })
	    messages = t.render(c)
...
#		fail_silently=False
 #          )
    ...
    return render_to_response('buy.html', {'form': form, 'buy':a }, context_instance=RequestContext(request))
